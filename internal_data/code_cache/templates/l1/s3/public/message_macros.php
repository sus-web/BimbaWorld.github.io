<?php
// FROM HASH: 8d0f5ce694e2cb5fe8a315ae7cbe285f
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__finalCompiled .= '
		...
		'itemprop' => 'name',
				...
				';
	if ($__vars['user']['user_id'] AND $__templater->method($__vars['xf']['visitor'], 'canStartConversationWith', array($__vars['user'], ))) {
		$__finalCompiled .= '
					<a href="' . $__templater->func('link', array('conversations/add', '', array('to' => $__vars['user']['username'], ), ), true) . '" data-xf-click="' . 'overlay' . '" class="xm-icon button--link"><i class="fas fa-envelope" aria-hidden="true" data-xf-init="tooltip" title="' . 'Start conversation' . '"></i></a>
				';
	}
	$__finalCompiled .= '
			</h4>
	...
	if ($__vars['user']['user_id']) {
		...
		if (strlen(trim($__compilerTemp1)) > 0) {
			...
			if ($__templater->func('property', array('xentr_user_info_block_collapse', ), false) == 'yes') {
				$__finalCompiled .= 'message-userExtras-body message-userExtras--collapsible';
			}
			$__finalCompiled .= '">
				' . $__compilerTemp1 . '
				</div>
