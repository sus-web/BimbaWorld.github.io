...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	';
		...
		$__templater->modifySideNavHtml(null, '
			<div class="block">
				<div class="block-container">
					<h3 class="block-header">' . 'Списки тем' . '</h3>
		...
		$__finalCompiled .= '
		';
		$__templater->setPageParam('sideNavTitle', 'Списки тем');
		$__finalCompiled .= '
	';
	}
	$__finalCompiled .= '
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<a class="' . $__templater->escape($__vars['baseClass']) . ' ' . (((($__vars['pageSelected'] == 'started') AND ($__vars['xf']['visitor']['user_id'] == $__vars['user']['user_id']))) ? $__templater->escape($__vars['selectedClass']) : '') . '"
		href="' . $__templater->func('link', array('find-threads/started', ), true) . '" rel="nofollow">' . 'Мои темы' . '</a>
	<a class="' . $__templater->escape($__vars['baseClass']) . ' ' . (($__vars['pageSelected'] == 'contributed') ? $__templater->escape($__vars['selectedClass']) : '') . '"
		href="' . $__templater->func('link', array('find-threads/contributed', ), true) . '" rel="nofollow">' . 'Темы с моими сообщениями' . '</a>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['pageSelected'] == 'started') {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Темы, созданные пользователем ' . $__templater->escape($__vars['user']['username']) . '');
...
';
		...
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Темы, с сообщениями пользователя ' . $__templater->escape($__vars['user']['username']) . '');
		$__templater->pageParams['pageNumber'] = $__vars['page'];
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Темы, не имеющие ответов');
		$__templater->pageParams['pageNumber'] = $__vars['page'];
		$__finalCompiled .= '
	';
