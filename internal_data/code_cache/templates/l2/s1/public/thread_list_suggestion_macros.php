...
'before_status_watch' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['xf']['visitor']['user_id']) {
		...
		if ($__vars['voteState'] == 'up') {
			$__finalCompiled .= '
				<li>
					<i class="structItem-status structItem-status--upvoted" aria-hidden="true" title="' . $__templater->filter('Позитивный голос', array(array('for_attr', array()),), true) . '"></i>
					<span class="u-srOnly">' . 'Позитивный голос' . '</span>
				</li>
			';
		} else if ($__vars['voteState'] == 'down') {
			$__finalCompiled .= '
				<li>
					<i class="structItem-status structItem-status--downvoted" aria-hidden="true" title="' . $__templater->filter('Негативный голос', array(array('for_attr', array()),), true) . '"></i>
					<span class="u-srOnly">' . 'Негативный голос' . '</span>
				</li>
			';
		}
		$__finalCompiled .= '
