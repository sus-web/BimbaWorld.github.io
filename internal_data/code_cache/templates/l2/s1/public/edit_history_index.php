...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped($__templater->escape($__vars['title']) . ' - ' . 'Редактировать историю');
	$__finalCompiled .= '
...
';
	...
	if ($__vars['editCount'] AND ($__vars['editCount'] > $__templater->method($__vars['editHistory'], 'count', array()))) {
		$__compilerTemp2 .= '
			...
			'_type' => 'cell',
			'html' => '
							' . 'Это содержимое было отредактировано ' . $__templater->filter($__vars['editCount'], array(array('number', array()),), true) . ' раз. Некоторые старые записи истории редактирования были удалены.' . '
						',
		))) . '
				';
	}
