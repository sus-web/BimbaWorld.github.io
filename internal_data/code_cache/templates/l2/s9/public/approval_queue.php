...
';
	$__vars['sortOrders'] = array('content_date' => 'Дата публикации', );
	$__finalCompiled .= '
	...
	if ($__vars['filters']['content_type']) {
		$__compilerTemp1 .= '
								<li><a href="' . $__templater->func('link', array('approval-queue', null, $__templater->filter($__vars['filters'], array(array('replace', array(array('content_type' => null, ), )),), false), ), true) . '"
									...
									<span class="filterBar-filterToggle-label">' . 'Тип содержимого' . $__vars['xf']['language']['label_separator'] . '</span>
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'В данный момент нет элементов, ожидающих одобрения.' . '</div>
';
	}
