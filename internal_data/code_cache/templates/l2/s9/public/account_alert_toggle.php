...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if (!$__vars['newUnreadStatus']) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Отметить оповещение прочитанным');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Отметить оповещение непрочитанным');
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
';
	...
	<div class="block-container">
		...
		'submit' => ((!$__vars['newUnreadStatus']) ? 'Отметить прочитанным(и)' : 'Отметить непрочитанным(и)'),
	), array(
		'rowtype' => 'simple',
	)) . '
	</div>
