...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Закрытые жалобы: ' . $__templater->func('date', array($__vars['timeFrame']['0'], ), true) . ' - ' . $__templater->func('date', array($__vars['timeFrame']['1'], ), true) . '');
	$__finalCompiled .= '
...
<div class="block">
	...
	<div class="block-container">
		<h2 class="block-tabHeader tabs">
			...
			<a href="' . $__templater->func('link', array('reports/closed', ), true) . '" class="tabs-tab is-active">' . 'Закрытые жалобы' . '</a>
		</h2>
	...
	if (!$__templater->test($__vars['reports'], 'empty', array())) {
		...
		$__finalCompiled .= '
				<div class="block-row">' . 'Закрытые жалобы за данный период отсутствуют.' . '</div>
			';
	}
	$__finalCompiled .= '
		</div>
		<div class="block-footer">
			<span class="block-footer-controls">
				' . $__templater->button('&larr; ' . 'Старее', array(
	...
	if ($__vars['page'] > 1) {
		$__finalCompiled .= '
					' . $__templater->button('Новее' . ' &rarr;', array(
			'href' => $__templater->func('link', array('reports/closed', null, array('page' => $__vars['page'] - 1, ), ), false),
		), '', array(
		)) . '
				';
