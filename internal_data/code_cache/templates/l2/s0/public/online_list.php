...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Текущие посетители');
...
<div class="block">
	...
	if (!$__templater->test($__vars['activities'], 'empty', array())) {
			...
			foreach ($__vars['activities'] AS $__vars['activity']) {
				...
				if ((!$__vars['activity']['User']) OR $__templater->method($__vars['activity']['User'], 'canViewCurrentActivity', array())) {
					...
					if ($__vars['activity']['description']) {
						...
						$__finalCompiled .= '
												<span title="' . $__templater->escape($__vars['activity']['controller_name']) . '::' . $__templater->escape($__vars['activity']['controller_action']) . '">' . 'Просматривает неизвестную страницу' . '</span>
											';
					} else {
						$__finalCompiled .= '
												' . 'Просматривает неизвестную страницу' . '
											';
					}
					$__finalCompiled .= '
											</li>
				...
				if (($__vars['activity']['view_state'] == 'error') AND $__templater->method($__vars['xf']['visitor'], 'canBypassUserPrivacy', array())) {
					$__finalCompiled .= '
						...
						'title' => $__templater->filter('Просматривает сообщение об ошибке', array(array('for_attr', array()),), false),
					)) . '
											<span class="u-srOnly">' . 'Просматривает сообщение об ошибке' . '</span>
											</li>
										';
				}
				$__finalCompiled .= '
		...
		$__finalCompiled .= '
			<div class="block-row">' . 'Результаты не найдены.' . '</div>
		';
	}
	$__finalCompiled .= '
	</div>
