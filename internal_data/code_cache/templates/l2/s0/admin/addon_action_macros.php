	...
	if (!$__vars['errors']) {
		...
		if ($__vars['warnings']) {
			...
			<div class="block-body">
				...
				'label' => 'Я прочитал(а) предупреждения, и всё ещё хочу продолжить',
				'_type' => 'option',
			)), array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['warnings']) {
		...
		<div class="block-rowMessage block-rowMessage--warning block-rowMessage--iconic">
			' . 'Чтобы избежать возможных проблем, пожалуйста, устраните следующие предупреждения, прежде чем продолжить' . $__vars['xf']['language']['label_separator'] . '
			<ul>
				';
