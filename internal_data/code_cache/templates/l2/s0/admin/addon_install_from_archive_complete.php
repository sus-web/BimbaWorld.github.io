...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Результат обработки');
	$__finalCompiled .= '
...
';
	...
	<div class="blockMessage blockMessage--error">
		' . 'Один или несколько плагинов из этой группы не удалось установить/обновить. Нажмите или наведите указатель мыши на значок ошибки, чтобы просмотреть сведения о ней.' . '
	</div>
';
	} else {
		$__finalCompiled .= '
			...
			if ($__vars['result']['status'] == 'error') {
				...
				$__compilerTemp2 .= '
								<span data-xf-init="tooltip" data-trigger="hover focus click"
									title="' . $__templater->filter('Действие успешно завершено', array(array('for_attr', array()),), true) . '">
