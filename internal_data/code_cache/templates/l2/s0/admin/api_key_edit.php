...
';
	...
	if ($__templater->method($__vars['apiKey'], 'isUpdate', array())) {
		$__compilerTemp1 .= '
				' . $__templater->formRow('
					...
					' . $__templater->button('Посмотреть полный ключ' . '
					', array(
			...
			'class' => 'button--link',
					...
					' . $__templater->button('Сгенерировать новый ключ' . '
					', array(
		...
		$__compilerTemp2 .= '
			...
			'apiKey' => $__vars['apiKey'],
				...
				' . $__templater->formRow('
					...
					&middot;
					' . 'От ' . ($__templater->escape($__vars['apiKey']['Creator']['username']) ?: 'Н/Д') . '' . '
				', array(
			'label' => 'Создано',
		)) . '
	...
	<div class="block-container">
		...
		'label' => 'Заголовок',
		'explain' => 'Укажите заголовок для этого ключа, как это показано в списке API-ключей.',
	)) . '
