...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Массовая обработка тем');
	$__finalCompiled .= '
...
';
	...
	if (!$__vars['threadIds']) {
		$__compilerTemp1 .= '
					...
					<a href="' . $__templater->func('link', array('threads/list', null, array('criteria' => $__vars['criteria'], 'all' => true, ), ), true) . '">' . 'Посмотреть или отфильтровать совпадения' . '</a>
				';
	}
	...
	if ($__vars['hasPrefixes']) {
		$__compilerTemp3 .= '
					' . 'Если выбранная(ые) тема(ы) имеет(ют) какие-либо префиксы, которые недействительны в указанном форуме, то эти префиксы будут удалены.' . '
				';
	}
	...
	if ($__vars['prefixes']['prefixesGrouped']) {
		$__compilerTemp5 .= '
			...
			'explain' => 'Префикс будет применён только в том случае, если он корректен для форума, в котором находится тема или в который она будет перемещена.',
		)) . '
			';
	}
	$__compilerTemp6 = '';
	...
	<div class="block-container">
		...
		'explain' => 'Тип темы будет изменён только в том случае, если он действителен для форума, в котором эта тема находится или в который перемещается.',
	)) . '
