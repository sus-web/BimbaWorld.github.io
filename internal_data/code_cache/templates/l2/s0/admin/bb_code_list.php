...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Пользовательские BB-коды');
	$__finalCompiled .= '
			...
			foreach ($__vars['bbCodes'] AS $__vars['bbCode']) {
				...
				if ($__vars['exportView']) {
					$__compilerTemp3[] = array(
						...
						'tooltip' => ($__vars['bbCode']['addon_id'] ? 'BB-коды, принадлежащие этому плагину, не могут быть экспортированы как самостоятельные коды.' : ''),
						'_type' => 'toggle',
						'html' => '',
					);
				}
