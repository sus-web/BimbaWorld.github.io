...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['memberStat'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Добавить статистику');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
...
<code>return [$user->user_id => \\XF::language()->numberFormat($value)];</code>',
		...
		'explain' => 'Если здесь выбрано право, то эта статистика будет доступна только пользователям с таким правом.',
	)) . '
		...
		'readOnly' => (!$__templater->method($__vars['memberStat'], 'canEdit', array())),
			...
			', array(
		'rowtype' => 'input',
		'label' => 'Видимость обратного вызова',
		'explain' => 'При желании Вы можете установить обратный вызов PHP, чтобы убедиться, что эта статистика должна отображаться.<br />
<br />
...
<ol>
	<li>
		<code>\\XF\\Entity\\MemberStat $memberStat</code>
		<br />
		Это сущность статистики пользователя.
	</li>
</ol>
Обратный вызов должен вернуть или <code>true</code>, или <code>false</code>, где <code>false</code> будет препятствовать отображению статистики участника.',
	)) . '
		...
		'hint' => 'Если включено и имеется, то значение будет отображено для каждого пользователя.',
		'_type' => 'option',
	),
	array(
		...
		'hint' => (('Если включено, то 5 лучших пользователей из этой статистики будут отображены на вкладке "Обзор".' . ' ') . ((($__vars['xf']['development'] AND $__vars['memberStat']['addon_id'])) ? 'Значение этого поля не будет изменено при обновлении плагина.' : '')),
		'_type' => 'option',
	)), array(
		'label' => 'Настройки',
	)) . '
