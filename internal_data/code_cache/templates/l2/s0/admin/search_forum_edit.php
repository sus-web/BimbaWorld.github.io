...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['node'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Добавить форум для поиска');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Редактировать форум для поиска' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['node']['title']));
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
' . $__templater->form('
	...
	array(
		...
		'label' => 'Длина кэша',
		'explain' => 'Результаты поиска, включая их порядок, будут кэшироваться в течение указанного промежутка времени. Более низкое значение обеспечивает более высокую точность за счёт производительности.',
	)) . '
		</div>

		<h2 class="block-formSectionHeader">
			<span class="block-formSectionHeader-aligner">' . 'Критерии тем' . '</span>
		</h2>
		<div class="block-body">
