...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Трофеи');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить трофей', array(
		...
		if ($__templater->isTraversable($__vars['trophies'])) {
			foreach ($__vars['trophies'] AS $__vars['trophy']) {
				$__compilerTemp1 .= '
					...
					'hint' => 'Баллы' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['trophy']['trophy_points']),
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Пока не создано ни одного трофея.' . '</div>
';
	}
	$__finalCompiled .= '
