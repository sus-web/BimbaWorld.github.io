	...
	$__finalCompiled .= '
			<div class="block-outer-opposite quickFilter u-jsOnly"
				...
				<div class="inputGroup inputGroup--inline inputGroup--joined">
					<input type="text" class="input js-filterInput" placeholder="' . 'Отфильтровать' . $__vars['xf']['language']['ellipsis'] . '" />
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['addOn']['is_processing']) {
		$__finalCompiled .= '
						<span class="label label--error" data-xf-init="tooltip" title="' . $__templater->filter('Последнее действие' . $__vars['xf']['language']['label_separator'], array(array('for_attr', array()),), true) . ' ' . ($__templater->escape($__vars['addOn']['last_pending_action']) ?: $__templater->filter('Н/Д', array(array('for_attr', array()),), true)) . '">' . 'В ожидании действия' . '</span>
					';
	}
	$__finalCompiled .= '
				</h3>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['addOn'], 'isInstalled', array()) AND (!$__templater->method($__vars['addOn'], 'canUpgrade', array()))) {
		...
		if ($__templater->method($__vars['addOn'], 'isLegacy', array())) {
			...
			$__finalCompiled .= '
					<li class="addOnList-row-noSearch">
						<span class="label label--red" data-xf-init="tooltip"
							title="' . $__templater->filter('Некоторые файлы или директории этого плагина отсутствуют (' . $__templater->filter($__vars['addOn']['missing_files'], array(array('join', array(', ', )),), false) . '). Пожалуйста, загрузите повторно все файлы плагина.', array(array('for_attr', array()),), true) . '">
							' . 'Отсутствуют файлы' . '
						</span>
					</li>
				';
			...
			$__finalCompiled .= '
					<li class="addOnList-row-noSearch">
						<span class="label label--primary" data-xf-init="tooltip"
							title="' . $__templater->filter('Файлы плагина имеют некорректную версию. Пожалуйста, загрузите корректные файлы повторно.', array(array('for_attr', array()),), true) . '">
			...
			$__finalCompiled .= '
					<li class="addOnList-row-noSearch">
						<span class="label label--primary" data-xf-init="tooltip"
							title="' . $__templater->filter('Этот плагин ожидает изменения метаданных. Пожалуйста, выполните синхронизацию или перестроение, чтобы импортировать данные изменения.', array(array('for_attr', array()),), true) . '">
	...
	if ($__vars['addOn']['dev'] OR $__vars['addOn']['dev_url']) {
		...
		if ($__vars['addOn']['dev'] AND $__vars['addOn']['dev_url']) {
			$__finalCompiled .= '
						<span class="addOnList-row-noSearch">' . 'Разработка' . $__vars['xf']['language']['label_separator'] . '</span> <a href="' . $__templater->escape($__vars['addOn']['dev_url']) . '" target="_blank">' . $__templater->escape($__vars['addOn']['dev']) . '</a>
					';
		} else if ($__vars['addOn']['dev']) {
			$__finalCompiled .= '
						<span class="addOnList-row-noSearch">' . 'Разработка' . $__vars['xf']['language']['label_separator'] . '</span> ' . $__templater->escape($__vars['addOn']['dev']) . '
					';
		} else if ($__vars['addOn']['dev_url']) {
			$__finalCompiled .= '
						<span class="addOnList-row-noSearch">' . 'Разработка' . $__vars['xf']['language']['label_separator'] . '</span> <a href="' . $__templater->escape($__vars['addOn']['dev_url']) . '" target="_blank">' . $__templater->escape($__vars['addOn']['dev_url']) . '</a>
					';
		}
		$__finalCompiled .= '
				</li>
