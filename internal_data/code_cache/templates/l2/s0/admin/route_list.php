...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Роутинги');
	$__finalCompiled .= '
...
';
	...
	' . $__templater->button('Добавить роутинг' . $__vars['xf']['language']['ellipsis'], array(
		'class' => 'menuTrigger',
		'data-xf-click' => 'menu',
		'aria-expanded' => 'false',
		'aria-haspopup' => 'true',
		...
		<div class="menu" data-menu="menu" aria-hidden="true">
			<div class="menu-content">
				<h3 class="menu-header">' . 'Добавить роутинг' . $__vars['xf']['language']['ellipsis'] . '</h3>
	...
	if ($__templater->isTraversable($__vars['routeTypes'])) {
		foreach ($__vars['routeTypes'] AS $__vars['routeTypeId'] => $__vars['routeType']) {
			...
			if (!$__templater->test($__vars['routesGrouped'][$__vars['routeTypeId']], 'empty', array())) {
				...
				$__finalCompiled .= '
						<div class="block-row">' . 'Пока не создано ни одного элемента.' . '</div>
					';
			}
			$__finalCompiled .= '
				</li>
