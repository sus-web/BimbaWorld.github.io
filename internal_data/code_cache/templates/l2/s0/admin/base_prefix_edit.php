...
';
	...
	if ($__templater->method($__vars['prefix'], 'hasRelation', array('MasterDescription', ))) {
		$__compilerTemp1 .= '
			...
			'label' => 'Описание',
			'explain' => 'Если этот префикс требует дополнительного объяснения при просмотре связанного элемента, Вы можете ввести здесь описание, опционально, используя HTML.',
		)) . '
			';
	}
	$__compilerTemp2 = '';
	if ($__templater->method($__vars['prefix'], 'hasRelation', array('MasterUsageHelp', ))) {
		$__compilerTemp2 .= '
			...
			'label' => 'Справка по использованию префиксов',
			'explain' => 'Если Вы хотите объяснить участникам использование этого префикса при подготовке их контента, введите описание здесь. Вы можете использовать HTML.',
		)) . '
			';
	}
