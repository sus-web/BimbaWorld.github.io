...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['upgradeCheck']['error_code']) {
		...
		if (!$__vars['upgradeCheck']['branding_valid']) {
			$__compilerTemp1 .= '
						<li>' . 'Копирайт разработчиков был удалён, но Ваша лицензия не позволяет Вам это сделать.' . '</li>
					';
		}
		$__compilerTemp1 .= '
					';
		if ($__templater->isTraversable($__vars['upgradeCheck']['invalid_add_ons'])) {
			foreach ($__vars['upgradeCheck']['invalid_add_ons'] AS $__vars['addOnId'] => $__vars['reason']) {
				...
				if ($__vars['reason'] == 'using_newer') {
					$__compilerTemp1 .= '
							<li>' . 'Вы используете версию ' . $__templater->escape($__vars['upgradeCheck']['RelevantAddOns'][$__vars['addOnId']]['title']) . ', которая является более новой, чем позволяет использовать Ваша лицензия.' . '
							</li>
						';
	...
	if (strlen(trim($__compilerTemp1)) > 0) {
		...
		<div class="blockMessage blockMessage--error blockMessage--iconic">
			' . 'Следующие проблемы должны быть решены как можно скорее. Как только они будут устранены, <a href="' . $__templater->func('link', array('tools/upgrade-check', ), true) . '">проверьте обновления снова</a> для подтверждения.' . '
			<ul>
			' . $__compilerTemp1 . '
			</ul>
		</div>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if (!$__templater->method($__vars['upgradeCheck'], 'hasLicenseErrors', array())) {
		...
		if (!$__vars['upgradeCheck']['board_url_valid']) {
			$__compilerTemp1 .= '
						<li>' . '<a href="' . ($__templater->func('link', array('options/groups/basicBoard/', ), true) . '#boardUrl') . '">URL-адрес форума</a> не соответствует адресу, указанному в Вашей лицензии.' . '
						</li>
					';
		}
		$__compilerTemp1 .= '
		...
		if ($__vars['upgradeCheck']['license_expired']) {
			$__compilerTemp1 .= '
						<li>' . 'Срок действия Вашей лицензии истёк. У Вас не будет доступа к новым версиям и поддержке. Чтобы продолжить получать последние обновления и поддержку, Вы должны <a href="https://xenforo.com/customers" target="_blank">продлить</a> свою лицензию.' . '</li>
					';
		}
		$__compilerTemp1 .= '
				';
		if (strlen(trim($__compilerTemp1)) > 0) {
			...
			<div class="blockMessage blockMessage--warning blockMessage--iconic">
				' . 'Должны быть решены следующие проблемы. Как только они будут устранены, <a href="' . $__templater->func('link', array('tools/upgrade-check', ), true) . '">проверьте обновления снова</a> для подтверждения.' . '
				<ul>
				' . $__compilerTemp1 . '
				</ul>
			</div>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['xf']['visitor'], 'hasAdminPermission', array('upgradeXenForo', )) AND ($__vars['upgradeCheck']['available_updates'] AND (!$__templater->method($__vars['upgradeCheck'], 'hasLicenseErrors', array())))) {
		...
		if ($__templater->method($__vars['upgradeCheck'], 'hasAvailableUpdate', array('XF', ))) {
			$__compilerTemp1 .= '
						' . 'XenForo ' . $__templater->escape($__vars['upgradeCheck']['available_updates']['XF']['version_string']) . ' доступен.' . '
			...
			$__compilerTemp1 .= '
						' . 'Доступны следующие обновления для официальных плагинов:' . '
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['xf']['visitor'], 'hasAdminPermission', array('upgradeXenForo', )) AND ($__vars['upgradeCheck']['installable_add_ons'] AND (!$__templater->method($__vars['upgradeCheck'], 'hasLicenseErrors', array())))) {
		...
		if ($__vars['upgradeCheck']['installable_add_ons']) {
			$__compilerTemp1 .= '
						' . 'Следующие официальные плагины доступны для установки:' . '
						<ul class="listInline listInline--comma listInline--selfInline">
							';
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['showSuccess'] AND (!$__templater->method($__vars['upgradeCheck'], 'hasNotice', array()))) {
		...
		<div class="blockMessage blockMessage--success">
			' . 'Ваша версия XenForo и официальные плагины полностью обновлены. С Вашей лицензией нет никаких проблем.' . '
		</div>
	';
	}
	$__finalCompiled .= '
