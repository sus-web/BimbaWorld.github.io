<?php
// FROM HASH: 026f7a1f9a10f1e7f63ed9932cf53418
return array(
...
'full_body' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
		$__finalCompiled .= '
			<div class="';
	if (($__templater->func('property', array('xentr_messageUserInfoBlockPosition', ), false) == 'firstPostHorizontal') AND (!$__vars['thread']['type_data']['allow_voting'])) {
		$__finalCompiled .= 'message-user-inner';
	} else {
		$__finalCompiled .= 'message-inner';
	}
	$__finalCompiled .= '">
...
';
	...
	' . '
	<article class="message  ' . ($__vars['post']['User']['is_staff'] ? ' message-staffPost ' : '') . ' ' . (($__vars['post']['Thread']['user_id'] === $__vars['post']['user_id']) ? ' message-threadStarterPost ' : '') . $__templater->escape($__templater->renderExtension('extra_classes', $__vars, $__extensions)) . ' js-post ';
	if (($__templater->func('property', array('xentr_messageUserInfoBlockPosition', ), false) == 'firstPostHorizontal') AND ($__templater->method($__vars['post'], 'isFirstPost', array()) AND (!$__vars['thread']['type_data']['allow_voting']))) {
		$__finalCompiled .= 'is-first';
	} else {
	}
	$__finalCompiled .= ' js-inlineModContainer ' . ($__templater->method($__vars['post'], 'isIgnored', array()) ? 'is-ignored' : '') . ' ' . ($__templater->method($__vars['post'], 'isUnread', array()) ? ' is-unread' : '') . '"
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['post'], 'isUnread', array())) {
		...
		$__finalCompiled .= '
				<li><span class="message-newIndicator" title="' . 'Новые ответы' . '">' . 'Новое' . '</span></li>
			';
	}
	$__finalCompiled .= '
			';
	if ($__templater->func('property', array('xentr_user_info_block_collapse', ), false) == 'yes') {
		$__finalCompiled .= '
				<li><span id="collapse-' . $__templater->escape($__vars['post']['post_id']) . '" class="collapseTrigger collapseTrigger--block" data-xf-click="toggle" data-xf-init="tooltip" title="' . 'Информация о пользователе' . '" data-target="#js-post-' . $__templater->escape($__vars['post']['post_id']) . ' .message-userExtras"></span></li>
			';
	}
	$__finalCompiled .= '
			<li>
...
<div style="margin-top: 10px">
				';
	if ($__vars['post']['position'] == 0) {
		$__compilerTemp2 .= '
					';
		$__compilerTemp3 = '';
		$__compilerTemp3 .= '
						';
		$__vars['set'] = $__vars['post']['User']['Profile']['custom_fields'];
		$__compilerTemp3 .= '
						';
		$__compilerTemp4 = $__templater->method($__vars['xf']['app'], 'getCustomFields', array('users', 'contact', null, array('profile', ), ));
		if ($__templater->isTraversable($__compilerTemp4)) {
			foreach ($__compilerTemp4 AS $__vars['fieldId'] => $__vars['fieldDefinition']) {
				...
				if ($__templater->method($__vars['fieldDefinition'], 'hasValue', array($__vars['set'][$__vars['fieldDefinition']['field_id']], ))) {
					$__compilerTemp3 .= '
								';
					$__vars['fieldValue'] = $__templater->preEscaped($__templater->callMacro('custom_fields_macros', 'custom_field_value', array(
					...
					if ($__vars['fieldDefinition']['wrapper_template']) {
						$__compilerTemp3 .= '
									' . $__templater->button($__templater->filter($__templater->method($__vars['fieldDefinition'], 'getFormattedOutput', array($__vars['fieldValue'], )), array(array('raw', array()),), true), array(
							'href' => $__vars['set'][$__vars['fieldDefinition']['field_id']],
							'target' => '_blank',
						), '', array(
						)) . '
						...
						$__compilerTemp3 .= '
										...
										';
						if ($__vars['fieldDefinition']['match_type'] == 'url') {
							$__compilerTemp3 .= '
											' . $__templater->button($__templater->escape($__vars['fieldDefinition']['title']), array(
								'href' => $__vars['set'][$__vars['fieldDefinition']['field_id']],
								'target' => '_blank',
							), '', array(
							)) . '
