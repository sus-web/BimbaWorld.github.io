...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped(($__vars['active'] ? $__templater->escape($__vars['active']['title']) : 'Полезные пользователи'));
	$__finalCompiled .= '
...
';
	if ($__templater->test($__vars['memberStats'], 'empty', array())) {
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Нет полезных пользователей для отображения.' . '</div>
';
	} else {
		$__finalCompiled .= '
	<section class="block">
		...
		if ($__vars['active']) {
			...
			if ($__compilerTemp2) {
				$__finalCompiled .= '
						<li class="block-row">' . 'Ни один пользователь не соответствует указанным критериям.' . '</li>
					';
			}
			$__finalCompiled .= '
				</ol>
