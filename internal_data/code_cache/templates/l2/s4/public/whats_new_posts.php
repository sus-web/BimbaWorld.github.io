...
'code' => function($__templater, array $__vars, $__extensions = null)
{
			...
			foreach ($__vars['threads'] AS $__vars['thread']) {
				...
				if ($__vars['xf']['visitor']['user_id'] AND ((!$__vars['activityBarShown']) AND ($__vars['thread']['last_post_date'] < $__vars['xf']['session']['previousActivity']))) {
					...
					<div class="structItem structItem--note">
						<div class="structItem-cell">' . 'Темы ниже не обновлялись с момента Вашего последнего посещения.' . '</div>
					</div>
					';
	...
	';
		...
		if ($__vars['xf']['visitor']['user_id'] AND (($__vars['findNew']['filters']['unread'] AND ($__templater->func('count', array($__vars['findNew']['filters'], ), false) == 1)))) {
			$__finalCompiled .= '
				' . 'У Вас нет непрочитанных сообщений. Вы можете <a href="' . $__templater->func('link', array($__vars['latestRoute'], null, array('skip' => 1, ), ), true) . '" rel="nofollow">посмотреть все последние сообщения</a>.' . '
			';
		} else {
			$__finalCompiled .= '
