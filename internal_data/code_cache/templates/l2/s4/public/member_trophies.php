...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Трофеи пользователя ' . $__templater->escape($__vars['user']['username']) . '');
	$__finalCompiled .= '
...
<div class="block">
	...
	if (!$__templater->test($__vars['trophies'], 'empty', array())) {
		...
		$__finalCompiled .= '
			<div class="block-body block-row">' . '' . $__templater->escape($__vars['user']['username']) . ' пока не награждён(ена) ни одним трофеем.' . '</div>
		';
	}
	$__finalCompiled .= '
