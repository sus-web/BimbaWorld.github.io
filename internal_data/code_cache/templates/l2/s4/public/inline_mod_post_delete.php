...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Модерация - Удалить сообщения');
	$__finalCompiled .= '
...
';
	...
	if ($__vars['firstPostCount']) {
		$__compilerTemp1 .= '
				' . $__templater->formInfoRow('
					<div class="blockMessage blockMessage--important"><strong>' . 'Примечание' . $__vars['xf']['language']['label_separator'] . '</strong> ' . 'При удалении этих сообщений будет удалено ' . $__templater->filter($__vars['firstPostCount'], array(array('number', array()),), true) . ' тем.' . '</div>
				', array(
		)) . '
			';
	}
	...
	<div class="block-container">
		<div class="block-body">
			' . $__templater->formInfoRow('Вы действительно хотите удалить ' . $__templater->escape($__vars['total']) . ' сообщений?', array(
		'rowtype' => 'confirm',
	)) . '
