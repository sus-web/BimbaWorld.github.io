...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped(($__vars['isUnread'] ? 'Отметить прочитанным(и)' : 'Отметить непрочитанным(и)'));
	$__finalCompiled .= '
...
';
	...
	if ($__vars['isUnread']) {
		$__compilerTemp1 .= '
					' . 'Пожалуйста, подтвердите то, что Вы хотите отметить следующую переписку прочитанной' . $__vars['xf']['language']['label_separator'] . '
				';
	} else {
		$__compilerTemp1 .= '
					' . 'Пожалуйста, подтвердите то, что Вы хотите отметить следующую переписку непрочитанной' . $__vars['xf']['language']['label_separator'] . '
				';
	}
	...
	<div class="block-container">
		...
		'submit' => ($__vars['isUnread'] ? 'Отметить прочитанным(и)' : 'Отметить непрочитанным(и)'),
	), array(
		'rowtype' => 'simple',
	)) . '
	</div>
